...
export class Play extends Phaser.Scene
{
    ...
    create ()
    {
        ...
        this.GAME_RATIO = this.GAME_WIDTH / this.GAME_HEIGHT;

        
        // To make assets respondable, put them all inside to fix Phaser.Scale.RESIZE, using handleResize()
        this.board = this.add.container(0, 0);

        // Originally in init(), but needs to be placed here, after the board is created
        this.volumeButton();
        ...
        // Background image
        const background = this.add.image(this.gridConfiguration.x - 63, this.gridConfiguration.y - 77, "background").setOrigin(0);
        this.board.add(background);

        const titleText = this.add.text(this.GAME_WIDTH / 2, this.GAME_HEIGHT / 2,
            "Memory Card Game\nClick to Play",
    ...
    createGridCards ()
    {
        ...
        return gridCardNames.map((name, index) => {
            ...
            this.board.add(newCard.gameObject);
    ...
    startGame ()
    {
        ...
        const winnerText = this.add.text(this.GAME_WIDTH / 2, -1000, "YOU WIN",
            { align: "center", strokeThickness: 4, fontSize: 40, fontStyle: "bold", color: "#8c7ae6" }
        ).setOrigin(.5)
            .setDepth(3)
            .setInteractive();
        this.board.add(winnerText);


        const gameOverText = this.add.text(this.GAME_WIDTH / 2, -1000,
            "GAME OVER\nClick to restart",
        ...
        this.input.on(Phaser.Input.Events.POINTER_MOVE, (pointer) => {
            if (this.canMove) {
                // const card = this.cards.find(card => card.gameObject.hasFaceAt(pointer.x, pointer.y)); original version, off due to offset
                const { x, y } = this.getPointerInBoard(pointer);

                    const card = this.cards.find(card =>
                    card.gameObject.hasFaceAt(x, y)
                );
                    ...
                    if (this.cardOpened !== undefined) {
                        ...
                        card.flip(() => {
                            ...
                            if (this.lives === 0) {
                                ...
                                // Plays card animation, to remove them from the screen, to prevent hiding gameOverText
                                this.cards.forEach((card, index) => {
                                this.add.tween({
                                    targets: card.gameObject,
                                    duration: 500,
                                    delay: index * 80,
                                    y: this.GAME_HEIGHT + 300,
    ...
    // Handles the scale of the assets inside the container called "board"
    handleResize(gameSize) {
        const { width, height } = gameSize;

        let scale;
        if (width / height > this.GAME_RATIO) {
            scale = height / this.GAME_HEIGHT;
        } else {
            scale = width / this.GAME_WIDTH;
        }

        const displayWidth = this.GAME_WIDTH * scale;
        const displayHeight = this.GAME_HEIGHT * scale;
    ...
    // Fixes issue with the cards having an offset
    getPointerInBoard(pointer){
        const x = (pointer.x - this.board.x) / this.board.scaleX;
        const y = (pointer.y - this.board.y) / this.board.scaleY;
        return { x, y };
    }
}
